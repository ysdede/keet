 ...
 *   2. Real WAV file processing (`life_Jim.wav` from parakeet.js compat-tests assets)
 ...
 *
 * Real-audio behavior:
 *   - Prefer the local sibling `../parakeet.js/compat-tests/shared/assets/life_Jim.wav`
 *   - Fall back to the raw GitHub asset when the sibling fixture is absent
 *   - Skip the real-audio assertions cleanly if neither source is available
 *
 * Run: npm test
 */
...
describe('Real audio: life_Jim.wav', () => {
    ...
    beforeAll(async () => {
        ...
        try {
            ...
            audioDuration = 0;
            hasRealAudio = false;
            console.warn(`SKIP: Could not load life_Jim.wav: ${error instanceof Error ? error.message : String(error)}`);
        }
    });

    const skipIfNoRealAudio = (ctx: { skip: () => void }): boolean => {
        if (!hasRealAudio) {
            ctx.skip();
            return true;
        }
    ...
    it('should parse the WAV file correctly', (ctx) => {
        if (skipIfNoRealAudio(ctx)) return;
