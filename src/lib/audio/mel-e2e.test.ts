...
describe('Real audio: life_Jim.wav', () => {
    ...
    beforeAll(async () => {
        ...
        try {
            ...
            console.warn(`SKIP: Could not load life_Jim.wav: ${(e as Error).message}`);
            // Provide a minimal dummy valid 16kHz audio buffer so tests don't crash
            // (they will be skipped dynamically)
    ...
    it('should parse the WAV file correctly', (ctx) => {
        if (skipIfNoData()) return ctx.skip();
